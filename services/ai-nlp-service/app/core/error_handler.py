"""Manejo centralizado de errores con logging para AI/NLP Service.
...
class LLMProviderError(ServiceError):
    """Error al comunicarse con proveedores LLM."""

    def __init__(self, provider: str, message: str = "Proveedor LLM no disponible"):
        detail = f"Error con proveedor {provider}: {message}"
        super().__init__(message=detail, status_code=503)


class ExternalServiceError(ServiceError):
    """Error al comunicarse con un servicio externo (citas, catalogo)."""
